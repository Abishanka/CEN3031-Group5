...
void sqlcommand::addData(QString courseID, QString courseName, QString assignmentName, QString date){
    ...
    qry.addBindValue(QDateTime::fromString(date, "yyyy-MM-ddTHH:mm:ssZ"));
 ...
 vector<vector<QString>> sqlcommand::getData(QString yesterday, QString tomorrow){
     vector<vector<QString>> assignmentVector;
      QSqlQuery qry;
      qry.prepare("SELECT * FROM database where duedate > ? AND duedate < ?");
      //qry.prepare("SELECT * FROM database");
      qry.addBindValue(yesterday);
      qry.addBindValue(tomorrow);
      if(!qry.exec()){
